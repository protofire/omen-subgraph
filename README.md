...
Represents the liquidity mining campaign of a curated market

`id` - campaign address
`initialized` - returns true if the campaign has been initialized
`owner` - address of campaign owner
`startsAt`, `endsAt` - unix timestamps
`duration` - duration of campaign (in seconds)
`locked` - returns true if campaign has been locked by owner
`fpmm` - curated market
`rewardTokens`, `rewardAmounts` - array of tokens used as reward and corresponding amounts
`stakedAmount` - amount of tokens currently staked to earn rewards
`deposits` - array of deposit events
`withdrawals` - array of withdrawal events
`claims` - array of claim events
`recoveries` - array of recovery events
